...
interface Props {
  product: Product | PRODUCT_BY_SLUG_QUERYResult;
  className?: string;
  selectedOptions?: {
    weight?: any;
    size?: any;
...
const AddToCartButton = memo(
  ({ product, className, selectedOptions, displayPrice }: Props) => {
    ...
    const handleAddToCart = useCallback(() => {
      ...
      if (maxStock >= newTotalQty) {
        ...
        addItem(productWithOptions as any);
      ...
      <div className="w-full h-12 flex items-center">
        {itemCount ? (
          ...
          <Button
            ...
            variant={"outline"}
            className={cn(
              "w-full flex-1 rounded-md border-2 border-gofarm-green/20 hover:border-gofarm-green relative overflow-hidden group/button",
