...
export async function confirmAddress(
  ...
  try {
    ...
    const employee = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid && isEmployee == true && employeeRole == "callcenter"][0]`,
      { firebaseUid }
    );
...
export async function confirmOrder(
  ...
  try {
    ...
    const employee = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid && isEmployee == true && (employeeRole == "callcenter" || employeeRole == "incharge")][0]`,
      { firebaseUid }
    );
    ...
    try {
      if (order.firebaseUid) {
        ...
        console.warn(
          "Order missing firebaseUid, skipping notification:",
          orderId
        );
      }
...
export async function markAsPacked(
  ...
  try {
    ...
    const employee = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid && isEmployee == true && (employeeRole == "packer" || employeeRole == "incharge")][0]`,
      { firebaseUid }
    );
    ...
    await invalidateOrder(orderId, order.firebaseUid);
...
export async function markAsDelivered(
  ...
  try {
    ...
    const employee = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid && isEmployee == true && (employeeRole == "deliveryman" || employeeRole == "incharge")][0]`,
      { firebaseUid }
    );
...
export async function receivePaymentFromDeliveryman(
  ...
  try {
    ...
    const employee = await backendClient.fetch(
      `*[_type == "user" && firebaseUid == $firebaseUid && isEmployee == true && (employeeRole == "accounts" || employeeRole == "incharge")][0]`,
      { firebaseUid }
    );
...
export async function getOrdersForEmployee() {
  try {
    ...
    switch (employee.employeeRole) {
      case "callcenter":
        // Show orders up to confirmed status (call center confirms address and order only)
