...
export const getMyOrders = async (
  ...
  // Fallback to cookie-based orders
  try {
    const { cookies } = await import("next/headers");
    const cookieStore = await cookies();
    const localOrdersCookie = cookieStore.get(`gofarm-orders-${userId}`)?.value;
    if (localOrdersCookie) {
      const localOrders = JSON.parse(decodeURIComponent(localOrdersCookie));
      localOrders.sort((a: any, b: any) => new Date(b.orderDate).getTime() - new Date(a.orderDate).getTime());
      
      const offset = (page - 1) * limit;
      const paginatedOrders = localOrders.slice(offset, offset + limit);
      const totalCount = localOrders.length;
    ...
    console.error("Local orders load from cookie failed", e);
  }
